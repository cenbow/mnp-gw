...
public class NpcMessageValidationHandler {
	...
	public String validate(String userId, String password, String xml) {
		...
		// FIXME: validate msgs checksum
		BigInteger checkSum = npcDataType.getMessageFooter().getChecksum();
		NPCMessageType npcMsg = npcDataType.getNPCMessages();
		boolean isMsgShouldCheck = ! (npcMsg.getPortTerminated().isEmpty()  );
		boolean isWillCheckSum = checkSum != null && isMsgShouldCheck;
		if (isWillCheckSum) {
			final MutableInt msisdnCnt = new MutableInt();

			npcMsg.getPortTerminated().forEach(msg -> msg.getNumberDataBase().forEach(e -> msisdnCnt.increment())); // // 9001

			if (checkSum.intValue() != msisdnCnt.intValue()) { // "NPC1035E: Invalid Checksum {1} for the Number Quantity {2}"
				validationCode = "NPC1035E";
				validationResult = errorCodeMapper.get(validationCode).replace(replaceString1, checkSum.toString()).replace(replaceString2, msisdnCnt.toString());
