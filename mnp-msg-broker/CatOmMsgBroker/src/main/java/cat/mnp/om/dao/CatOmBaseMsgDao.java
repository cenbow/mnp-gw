 ...
 * @author HP-CAT
 */
public class CatOmBaseMsgDao extends MvnoMsgDao {

    private static final Logger logger = LoggerFactory.getLogger(CatOmBaseMsgDao.class);
    ...
    public String importMsg(Object msgObject) throws Exception {
        ...
        Map jdbcOutputParameters = jdbcCaller
                .declareParameters(callParameterList.toArray(new SqlParameter[0]))
                .execute(jdbcInputParameters);
        return (String) jdbcOutputParameters.get(errorParameterName);
    }

    public List<CatOmBaseMsg> mergeMsg() throws Exception {
        Map jdbcOutputParameters = jdbcCaller
                .declareParameters(callParameterList.toArray(new SqlParameter[0]))
                .execute();
