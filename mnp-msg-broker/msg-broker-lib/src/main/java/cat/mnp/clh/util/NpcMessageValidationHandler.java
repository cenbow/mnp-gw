...
import com.telcordia.inpac.ws.ProcessNPCMsg;
...
public class NpcMessageValidationHandler {
    ...
    public String validate(ProcessNPCMsg processNpcMsg) {
        String userId = processNpcMsg.getUserId();
        String password = new String(Base64.decodeBase64(processNpcMsg.getPassword()));
        String xmlMsg = processNpcMsg.getXmlMsg();
        ...
        } else if (!password.equals(userMapper.get(userId).get("password"))) {
        ...
        try {
            ...
            NPCMessageData npcMessageData = NpcMessageUtils.unMarshal(getJaxbUnMarshaller(), xmlMsg);
        ...
        if (messageHeader.getReceiver() != null && !messageHeader.getReceiver().equals(userMapper.get(userId).get("receiver"))) {
        ...
        if (!messageHeader.getSender().equals(userMapper.get(userId).get("sender"))) {
