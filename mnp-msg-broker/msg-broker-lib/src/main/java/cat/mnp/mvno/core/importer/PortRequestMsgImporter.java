 ...
 * @author HP-CAT
 */
public class PortRequestMsgImporter extends MsgHandlerBase {

    private static final Logger logger = LoggerFactory.getLogger(PortRequestMsgImporter.class);
    ...
    public void processMsg(Message msg) throws Exception {
        ...
        List msgList = NpcMessageUtils.listPortRequestAck(npcMessages);
        msgList.addAll(NpcMessageUtils.listPortRequestDonor(npcMessages));

        if (msgList.size() > 0) {
            logger.info("Importing PortReq Msg size: {}", msgList.size());
            getMvnoMsgDao().processMsg(mqHeaders, messageHeader, msgList);
        } else {
            logger.error("Invalid PortReq Msg: {}", messageHeader);
        }
    }
}
